...
class ParseRawDataUseCase
{
    public function parse(FilenameQuery $query): RawMediaData
    {
        ...
        $baseName = preg_replace('/\b(bluray|dvd|dvdrip|hdrip|webrip|web-dl|webdl|web)\b/i', '', $baseName);
        $baseName = preg_replace('/\b(x264|x265|xvid|divx|hevc|h264|h265|h.264|h.265)\b/i', '', $baseName);
        $baseName = preg_replace(
            '/\b(5.1|7.1|aac|ac3|dts|dd5.1|dd7.1|dual audio|dual-audio|dualaudio|truehd)\b/i',
            '',
            $baseName
        );
        $baseName = preg_replace('/\b(ass|srt|sub|subrip|subtitles)\b/i', '', $baseName);

        $this->setExtension($rawData, $baseName);
        $this->setYear($baseName, $rawData);
        $this->setSeason($baseName, $rawData);
        $season = $rawData->getSeason();
        if ($season === null && $query->getType() === FilenameQuery::TYPE_PATH) {
